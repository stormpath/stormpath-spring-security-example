...
/**
 * A class representing the <a href="http://en.wikipedia.org/wiki/INI_file">INI</a> text configuration format that can override
 * already presented values when subsequent ini resources are loaded.
 * <p/>
 * An OverridableIni instance is a map of {@link OverridableIni.Section Section}s, keyed by section name.  Each
...
public class OverridableIni implements Map<String, OverridableIni.Section> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OverridableIni.class);
    ...
    /**
     * Creates a new empty {@code OverridableIni} instance.
     */
    public OverridableIni() {
        this.sections = new LinkedHashMap<String, Section>();
    }

    /**
     * Creates a new {@code OverridableIni} instance with the specified defaults.
     *
     * @param defaults the default sections and/or key-value pairs to copy into the new instance.
     */
    public OverridableIni(OverridableIni defaults) {
        this();
        if (defaults == null) {
    ...
    /**
     * Returns the names of all sections managed by this {@code OverridableIni} instance or an empty collection if there are
     * no sections.
     *
     * @return the names of all sections managed by this {@code OverridableIni} instance or an empty collection if there are
     *         no sections.
     */
    ...
    /**
     * Returns the sections managed by this {@code OverridableIni} instance or an empty collection if there are
     * no sections.
     *
     * @return the sections managed by this {@code OverridableIni} instance or an empty collection if there are
     *         no sections.
     */
     ...
     * @throws ConfigurationException if the path cannot be loaded into an {@code OverridableIni} instance.
     */
    public static OverridableIni fromResourcePath(String resourcePath) throws ConfigurationException {
        if (!Strings.hasLength(resourcePath)) {
            throw new IllegalArgumentException("Resource Path argument cannot be null or empty.");
        }
        OverridableIni ini = new OverridableIni();
    ...
    public void loadFromPath(String resourcePath) throws ConfigurationException {
        InputStream is;
        try {
            is = new FileInputStream(ResourceUtils.getFile(resourcePath));
        } catch (IOException e) {
    ...
    private void addSection(String name, StringBuilder content) {
        if (content.length() > 0) {
            ...
            if (cleaned != null) {
                ...
                if (!section.isEmpty()) {
                    Section existentSection = getSection(name);
                    if(existentSection != null) {
                        for(String key : section.keySet()) {
                            existentSection.put(key, section.get(key));
                        }
                        sections.put(name, existentSection);
                    } else {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof OverridableIni) {
            OverridableIni ini = (OverridableIni) obj;
    ...
    public String toString() {
        if (CollectionUtils.isEmpty(this.sections)) {
            ...
            int i = 0;
            for (OverridableIni.Section section : this.sections.values()) {
                if (i > 0) {
                    sb.append(",");
                }
    ...
    /**
     * An {@code OverridableIni.Section} is String-key-to-String-value Map, identifiable by a
     * {@link #getName() name} unique within an {@link OverridableIni} instance.
     */
    public static class Section implements Map<String, String> {
        ...
        private Section(String name, String sectionContent) {
            ...
            if (Strings.hasText(sectionContent) ) {
